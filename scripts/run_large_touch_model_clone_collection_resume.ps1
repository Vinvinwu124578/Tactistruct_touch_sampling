...
$Name = "isaac_tactile_gym_clone_touch_model_large1000_fast_stable"
$OutRoot = Join-Path $Root "outputs\touch_model_clone_data"
$LogDir = Join-Path $Root "outputs\touch_model_clone_logs"
$WarpCache = Join-Path $Root ("outputs\warp_cache\touch_model_clone_large1000_resume_{0}" -f (Get-Random))
$StdoutLog = Join-Path $LogDir "collect_clone_large1000_resume_ps_stdout.log"
$StderrLog = Join-Path $LogDir "collect_clone_large1000_resume_ps_stderr.log"
...
Write-Log ("[INFO] resume dataset=touch_charts_gt_{0}" -f $Name)
Write-Log ("[INFO] output_root={0}" -f $OutRoot)
Write-Log ("[INFO] WARP_CACHE_PATH={0}" -f $env:WARP_CACHE_PATH)

if ($ProbeOnly) {
    Write-Log "[PROBE] resume launcher script reached logging successfully."
    exit 0
}
