...
$WarpCache = Join-Path $Root ("outputs\warp_cache\touch_model_clone_large1000_fast_stable_{0}" -f (Get-Random))
$StdoutLog = Join-Path $LogDir "collect_clone_large1000_fast_stable_ps_stdout.log"
$StderrLog = Join-Path $LogDir "collect_clone_large1000_fast_stable_ps_stderr.log"
...
if ($ProbeOnly) {
    Write-Log "[PROBE] launcher script reached logging successfully."
    exit 0
}
